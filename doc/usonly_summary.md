...
#### 技术实现
- **Leaflet + react-leaflet**：开源地图库，动态导入避免 SSR 问题
- **OpenStreetMap**：免费地图数据源
- **L.divIcon**：自定义头像图标，32x32 像素圆形
- **useMap hook**：获取地图实例，程序控制地图
- **flyTo 方法**：平滑定位动画（duration: 0.5s）
- **MapInstanceSetter 组件**：地图实例设置
- **MapController 组件**：地图中心控制

#### 移动端优化
- 地图高度从 384px 减小到 256px，减少误触区域
...
#### 数据库变更
- Post 表新增字段：`latitude`、`longitude`、`location`
