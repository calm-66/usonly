    ...
    <main className="min-h-screen bg-white pb-20">
      ...
      <header className="bg-white border-b border-gray-100 sticky top-0 z-40">
        <div className="max-w-[500px] mx-auto px-4 py-3 flex items-center justify-between">
          <h1 className="text-lg font-bold text-primary">配对中心</h1>
        ...
        <div className="flex bg-gray-50 rounded-lg p-1 mb-4 border border-gray-100">
          <button
            ...
            className={`flex-1 py-2 text-sm font-medium rounded-md transition ${
              activeTab === 'search' ? 'bg-white text-primary shadow-sm' : 'text-gray-500'
            }`}
          >
            搜索用户
          </button>
          <button
            ...
            className={`flex-1 py-2 text-sm font-medium rounded-md transition ${
              activeTab === 'requests' ? 'bg-white text-primary shadow-sm' : 'text-gray-500'
            }`}
          >
            收到的请求
            {/* 只有当有请求且用户未查看时才显示徽章 */}
            ...
            {searchResults.length > 0 && (
              <div className="space-y-2">
                {searchResults.map((result) => (
                  ...
                  >
                    <div>
                      <p className="font-medium text-primary text-sm">{result.username}</p>
                      <p className="text-xs text-gray-500">{result.email}</p>
                    </div>
                    <button
                      onClick={() => handleSendRequest(result.id)}
                      disabled={loading || !!result.partnerId}
                      className="px-3 py-1.5 bg-primary text-white text-sm rounded-lg hover:bg-primary-hover transition disabled:opacity-50"
                    >
            ...
            {requests.length === 0 ? (
              ...
              <div className="space-y-2">
                {requests.map((request) => (
                  ...
                  >
                    <div>
                      <p className="font-medium text-primary text-sm">{request.sender.username}</p>
