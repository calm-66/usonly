...
系统已集成 ZPay mapi.php API，支持移动端 H5 支付，优化用户体验。

### 功能说明

**移动端（手机浏览器）**：
- 点击"立即支付"后，直接跳转到支付宝/微信 App 的支付确认页面
- 无需截图再打开相册扫码
...
1. **设备检测**：根据 User-Agent 自动识别移动端/PC 端
2. **API 接口**：`/api/payment/create-h5` 调用 ZPay mapi.php
3. **参数说明**：
   - `device`: `mobile`（移动端）或 `pc`（PC 端）
   - `clientip`: 用户真实 IP 地址
   - `type`: `alipay` 或 `wxpay`
